...
\large
\begin{tabular}{ll}
  Supervisors:& Miguel Afonso Dias de Ayala Botto \\
              & Fernando Vírgilio Pires de Oliveira
\end{tabular}
...
\begin{tabular}{r l}
  ...
  Supervisor:  & Eng. Fernando Vírgilio Pires de Oliveira \\
...
\begin{spacing}{1}
The development of this project was assisted by AI tools, namely:
\begin{itemize}
    \item \href{https://marketplace.visualstudio.com/items?itemName=ltex-plus.vscode-ltex-plus}{LTeX+} - grammar/spell checking using LanguageTool: VS Code extension used for text revision
    \item \href{https://consensus.app/}{Consensus} - AI for research: Used for bibliographic search
    \item \href{https://claude.ai/}{Anthropic Claude Sonnet 4.6}: Used for writing LaTeX, MATLAB, and Python code, and for text revision
    \item \href{https://chatgpt.com/}{OpenAI ChatGPT GPT-4o}: Used for writing LaTeX, MATLAB, and Python code, and for text revision
\end{itemize}
\end{spacing}
\end{flushleft}
\vfill\newpage
...
Ao Fernando Oliveira e ao professor Miguel Botto, que me trouxeram a este projeto e sempre me guiaram. Foram uma ajuda indispensável, incansáveis no seu apoio. Foi um prazer trabalhar convosco. Obrigado por tudo!

\vspace{5mm}

Ao António Correia, enquanto responsável pelo Programa de Investigação MEstr, que possibilitou este trabalho. À equipa do NESDE, que tão bem me acolheu, e pela dedicação com que desenvolve o seu trabalho todos os dias, é um prazer conviver convosco. Ao Paulo Candeias e ao Fernando Oliveira, cuja generosidade em partilhar o seu conhecimento foi inestimável  (não sou chato de propósito... é só curiosidade!). Ao Francisco Vaz, que me acompanhou nesta jornada, e alimentou horas de almoço com interessantes discussões. Obrigado a todos!

\vspace{5mm}

Aos professores do Técnico, por quem nutro bastante respeito pelo seu empenho na nobre tarefa que é o ensino. Obrigado.

\vspace{5mm}

À minha família, aos meus pais, João e Cristina, e à minha querida Maria, que sempre me apoiaram. Grato por tudo.
...
\begin{minipage}{0.77\textwidth}
    ...
    This research presented in this dissertation was funded by LNEC under the Research Programme 'Modelação do Comportamento Estrutural - MEstr', and the work developed was made possible through the participation of two research institutions, LNEC and IDMEC-Técnico, which provided all the necessary resources.

     \vspace{1cm}

    \includegraphics[height=1.7cm]{Figures/logo_lnec.jpg}\hfill
    \includegraphics[height=1.7cm]{Figures/idmec.jpg}
...
Nesta dissertação, um modelo computacional da plataforma sísmica uniaxial do LNEC é utilizado para comparar o desempenho, em termos de espetro de resposta, de três estratégias de controlo distintas: 1) o método iterativo atualmente utilizado, designado por  OLI (\emph{online iteration}), que consiste num controlo pseudo-adaptativo offline em anel aberto com o ensaio a decorrer em anel fechado com um controlador PI (proporcional-integral); 2) um controlador sintonizado PID-F (proporcional-integral-derivativo-filtrado); 3) um controlador ótimo de formulação linear-quadrática-integral (LQI).
...
\textbf{Palavras-chave:} Engenharia Sísmica, Plataforma Sismica,  Controlo de Sistemas, Identificação de Sistemas, Identificação em Anel Fechado
\end{otherlanguage}
%%%%%
% LTeX: enabled=true
%\clearpage
...
\textbf{Keywords:} : Earthquake Engineering, Shaking Table, Control Systems, System Identification, Closed-loop Identification
...
Earthquakes are among the most destructive natural hazards, causing severe loss of life, extensive property damage, and significant socio-economic disruption. Reducing seismic risk requires, among other measures, limiting the structural vulnerability of the built environment, which is pursued through the development of adequate building codes and improved construction techniques. Shaking table testing plays a fundamental role in supporting both endeavours, providing the experimental evidence needed to assess structural performance under realistic seismic loading.
...
\begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Uniaxial shaking table with a coupled 2DOF system (red and orange carts). The platen is shown in light green, and the actuators in blue.}
...
\begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Triaxial shaking table with a scaled test specimen of a two-story building. The rigid elements (platen and linkages) are shown in red, and the actuators in black and silver.}
    \label{fig_3D_render_triaxial}
\end{subfigure}
    \caption{3D rendering of LNEC's shaking tables}
\end{figure}
...
\pagebreak
\section{Objectives}
The objective of this work is to implement a controller to ensure that the seismic platform reproduces the test seismic signal (reference/earthquake) as accurately as possible. This objective will be attained under the following assumptions:
\begin{itemize}
...
\begin{align}
    Q_n =& E\left[ w(t) w(t)^\intercal\right] \\
    R_n =& E\left[ v(t) v(t)^\intercal\right] 
\end{align}

The Kalman estimator:
\begin{align}   
    \dot{\widehat{x}}(t) =& A \widehat{x}(t) + B u(t) + L_K \left( y(t) - C \widehat{x}(t) \right) \\
    \widehat{y}(t) =& C \widehat{x}(t) 
\end{align}

Computes state estimate $\widehat{x}(t)$ that minimizes the steady-state error covariance:
\begin{equation}
    P = \lim_{t \rightarrow \infty} E \left( (x(t) - \widehat{x}(t)) (x(t) - \widehat{x}(t))^\intercal \right)
\end{equation}
...
\begin{itemize}
    ...
    \item The data sequences $r_{2}(t)$ and $e_0(t)$ for the pairs of examples are the same, with the disturbance input $e(t)$ in each one resulting from a scaling of $e_0(t)$:  $e(t) = k \cdot e_0(t)$;
...
$k_p$ & Proportional gain of the controller & \SI{129.93}{\volt\per\meter} \\ \hline
$\tau_{sv}$ & Time-delay parameter of the servo-valve transfer function & \SI{0.0246}{\second} \\ \hline
$k_{sv}$ & Servo-valve gain & -- \\ \hline
$k_q$ & Valve flow gain & -- \\ \hline
$k_{sv}k_q$ & Total valve flow gain & \SI{0.0019345}{\meter\cubed\per\second\per\volt} \\ \hline
$k_c$ & Valve pressure-flow gain & -- \\ \hline
$C_l$ & Total leakage coefficient of the piston & -- \\ \hline
$k_{Pl}$ & $= k_c + C_l$ & \SI{1.67401e-10}{\meter\cubed\per\second\per\pascal} \\ \hline
$A$ & Area of the fluid under compression in the actuator & \SI{0.012456}{\meter\squared} \\ \hline
$V_t$ & Total volume of the fluid under compression in the actuator & \SI{0.002659}{\meter\cubed} \\ \hline
$\beta_e$ & Effective bulk modulus of the system & \SI{193716280}{\pascal} \\ \hline
$K_h$ & Oil-column frequency of the actuator $\left( = 4 \cdot \beta_e \cdot A^2 / V_t \right)$ & \SI{45213171683}{\pascal\per\meter} \\ \hline
...
$m_p$ & Mass of the platen & \SI{1975.1}{\kilogram} \\ \hline
$c_t$ & Combined damping force of the actuator and the platen & \SI{5780.0}{\newton\second\per\meter} \\ \hline
...
\subsection{A Model for LNEC Uniaxial Shaking Table with a Coupled 2DOF System}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Modelling/ST1D_block_diagram.png}
        % \begin{tikzpicture}[
...
% \end{equation}
where the parameters $K_p, K_i, K_d$, and $T_f$, seen in table \ref{tab:pidf_params}, were computed using MATLAB's \textit{pidtune} function, with the tuning option \textit{'DesignFocus'} set to \textit{'reference-tracking'}, and target cutoff frequency set to 20Hz. These results are labelled as "tuned" in figures \ref{fig_response_spectrum_of_3} through to \ref{fig_Platen_Displacement_Tracking_Error}. The code implementation can be found at the following GitHub repository \href{https://github.com/c-radicallis/Controlo_Plataforma_Sismica/blob/main/uniaxial_table_model/simulation_and_plotting.m}{location}.
...
For the shaking table with a coupled 2DOF system, the following state space representation was adopted, with the state vector $x(t) = [Q_{sv} ,\, F_p ,\, x_T ,\, x_1 ,\, x_2 ,\, \dot{x}_T ,\, \dot{x}_1 ,\, \dot{x}_2 ]^{T}$, and the control action being the signal to the servo-valve $u(t) = i_{sv}$: 
% \begin{align}
...
\begin{equation}
y(t) = [0\, 0\, 1\, 0\, 0\, 0\, 0\, 0] \cdot [Q_{sv} ,\, F_p ,\, x_T ,\, x_1 ,\, x_2 ,\, \dot{x}_T ,\, \dot{x}_1 ,\, \dot{x}_2 ]^{T} = x_T
\end{equation}
...
For the tracking loop shown in figure \ref{fig_LQI_loop}, where \emph{sys} is the state space model presented in section \ref{section_ss_model}: \vspace{-5mm}
\begin{figure}[H]
    \centering
...
\begin{minipage}{0.49\textwidth}
    ...
    \caption{Bode plots of $G_ {xT/x_{\text{ref}}}(s)$ for the default controller, tuned PID-F, and optimal state-feedback controller with $Q_{x_i}=10^9$.}
...
\pagebreak
\subsection{LNEC's Control Strategy: OLI method}\label{sec_LNEC_method} \vspace{-2mm}
...
\subsubsection{The functioning of LNEC's \textit{Adapt} software} \vspace{-2mm}
...
The process starts with the definition of the input files on the project menu, shown in figure \ref{fig_Adapt_Project_Parameters}: \vspace{-2mm}
\begin{figure}[H]
    \centering
...
\clearpage
\subsubsection{Simulating the driver adaptation process}\label{sec_Simulating_OLI}
...
\paragraph{Application example: Discussion of Method \& Results} \label{sec_OLI_Application_example}
In this application example of the OLI method, the target signal is the L'Aquila reduced-scale earthquake, and the test specimen modal properties are those described in Section \ref{sec_Simulating_OLI}. For the driver generation settings, which were kept throughout, coherence threshold was set to 0.9, maximum frequency compensation was set to 25Hz, and the correction factor was set to 0.65. For the identification, various settings were used, while window overlap was set to 0.5 and preserved as such, and the results are shown in figure \ref{bode_compare}, where a large number of windows results in a poor identification; in figure \ref{bode_compare_better_id}, where using 1024 Low Sidelobe windows results in an improved approximation of the transfer function, but identification still is unable to capture the resonance/anti-resonance at 1.5 Hz; and in figure \ref{FRFvsModelTF}, where the increasedg the width of the windows, in this case, using 20 Hanning windows results in a much better identification, now capturing the resonance/anti-resonance at around 1.5 Hz (however, the use of so few windows likely wouldn't work if the identification was being done with noisy experimental data).

The identified FRF which largely matches the model, shown in figure \ref{bode_compare_better_id}, yields adapted drivers which mostly converge to the target response spectra. Nevertheless, three iterations are still required to approximate the target, and the method does not perform as well as the LQI and PID-F controllers.

The identified FRF (Figure \ref{bode_compare}) is of lower quality due to the settings selected in \emph{Adapt}, and the resulting response spectrum, shown in figure \ref{Response_Spectra_2drivers}, after three iterations is unsatisfactory. This highlights two key limitations of the method. First, the outcome depends on the user selecting appropriate settings, yet no automatized and systematic procedure exists to guarantee an optimal choice of settings. Second, unlike in this simulation study where the true system is known and the FRF can be verified against it (Figure \ref{bode_compare_better_id}), in practice the user has no such reference, and must instead rely on an informed judgement based on the information available in \emph{Adapt} and other knowledge of the shaking table and test specimen dynamics.
...
\begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{2048 Low Sidelobe windows}
    \label{bode_compare}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\linewidth]{Adapt/bode_compare_better_id.png}
    \caption{1024 Low Sidelobe windows}
    \label{bode_compare_better_id}
\end{subfigure}
\\
\begin{subfigure}{0.5\textwidth}
    \includegraphics[width=\linewidth]{Adapt/FRFvsModelTF.png}
    \caption{20 Hanning windows}
    \label{FRFvsModelTF}
\end{subfigure}
\caption{Bode plot of model of the system (from Section \ref{Modelling Shaking Table Dynamics}) used in the simulations, and the identified frequency response function in \emph{Adapt} for various identification settings}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Shaking table response spectra: Target, tuned PID-F controller, LQI controller, and the 3 iterations of OLI method using FRF from the worse identification (shown in figure \ref{bode_compare})}
...
\begin{figure}[H]
    ...
    \caption{Shaking table response spectra: Target, tuned PID-F controller, LQI controller, and the 3 iterations of OLI method using FRF from the better identification (shown in figure \ref{bode_compare_better_id})}
...
The 2DOF system coupled to the platen had the modal properties described in Section \ref{sec_Simulating_OLI}. 

For the OLI method, the identification settings were 0.5 window overlap for 20 Hanning windows, with FRF mode set to H3, no filtering, and displacement/acceleration cross-over frequency turned off, and the driver adaptation settings were kept as described in Section \ref{sec_OLI_Application_example}. For each earthquake 3 driver iterations were performed.

The PID-F controller was tuned with the settings described in Section \ref{sec_Tuned_PID-F}.

For the LQI controller, the performance index (Eq. \eqref{eq_LQI_cost_function}) had $R=10^{-9}$, and $Q$ with every entry set to zero, except $Q_{x_i}=10^3$, as to have a similar bandwidth to the PID-F controller for the sake of comparing both (see figure \ref{fig_Bode_LQI_Qe3}). 

The code implementation, along with the developed Python and MATLAB functions for format conversion between LNEC test files (\emph{.ltf / .tgt / .drv / .acq}) and standard text files, can be found at the following GitHub repository \href{https://github.com/c-radicallis/Controlo_Plataforma_Sismica/blob/main/uniaxial_table_model/Adapting_Driver_Signal/sim_plot_adapted_drivers.m}{location}.

The shaking table acceleration and displacement response spectrum MSE for each earthquake, for each control strategy is shown in figure \ref{excel_accel_resp_spectrum} and \ref{excel_disp_resp_spectrum}. The mean and median acceleration and displacement response spectrum MSE are shown in table \ref{tab:metrics_comparison}. To highlight how the platen response spectra obtained for each control strategy (and iteration, in the case of OLI method) varies across the frequency range, the results for the Erzikan earthquake in the fault normal direction are shown in figure \ref{erzikan_4th_N}. The response spectra for all other earthquakes are presented in appendix \ref{appendix:1DST_benchmark}.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Adapt/excel_accel_resp_spectrum.png}
    \caption{Acceleration response spectrum MSE for various earthquakes. (For the Erzikan earthquake (figure \ref{erzikan_4th_N}), an additional driver adaption iteration was performed, and the results shown are for the resulting 4th driver)}
...
\begin{figure}[H]
    ...
    \caption{Displacement response spectrum MSE for various earthquakes. (For the Erzikan earthquake (figure \ref{erzikan_4th_N}), an additional driver adaption iteration was performed, and the results shown are for the resulting 4th driver)}
...
\begin{figure}[H]
    ...
    \caption{Erzikan fault North-South response spectra. For this case a 4th adapted driver was computed, which yielded better results then the PID-F (unlike the 3rd driver used in every other performance comparison), but only in terms of acceleration response spectrum and in the fault normal direction.}
    \label{erzikan_4th_N}
\end{figure}

\section{ Conclusions: PID-F vs Optimal Control vs OLI Method}
The LQI controller achieves considerably more accurate replication of the target ground response spectra at low frequencies compared to the tuned PID-F, which exhibits undesired amplification in the same frequency band. Both tuned controllers outperform the OLI method. The trade-off of the LQI controller, compared to the PID-F, is a response spectrum that falls short of the target faster at higher frequencies.

The simulations suggest that the LQI controller can be tuned for very high cut-off frequencies without inducing low-frequency amplification, a limitation observed in the PID-F. However, the performance of a high-bandwidth LQI controller in practice may deviate from the simulation results, given the limitations of the model usend in the simulations.

Overall, the tuned controllers (PID-F and LQI) offer superior response spectrum replication compared to the OLI method currently employed at LNEC, while requiring less user intervention. The LQI controller yields the best performance in all cases. Furthermore, the proposed control strategies provide a systematic and automated approach to the control problem, and they may also be used in conjunction with the OLI method, which, being a combined open- and closed-loop strategy with no restrictions on the closed-loop controller, can incorporate the benefits of improved closed-loop control alongside open-loop driver signal adaptation.

Finally, the performance of all three approaches is dependent on accurate system identification, and the sensitivity of each method to identification accuracy warrants further investigation.
...
The control strategy currently implemented at LNEC is the OLI control approach. For the majority of applications, the PI controller parameters are tuned in advance for a specific test configuration and subsequently kept fixed throughout most experimental campaigns.

\subsection{Implementation of the PID-F Controller in the Shaking Table Control Software}

In order to test the proposed controllers, the existing LNEC shaking table control software was modified to include them. The shaking table control software is a LabVIEW application, chosen for its native integration with the National Instruments (Fig. \ref{fig_labview_scheme}). The software architecture consists of three layers: a user interface running on a host computer (HMI), a Real-Time controller that executes the supervisory application (National Instruments PXI Real-Time Controller), and an FPGA (PXI-7841R) on which the control and data acquisition loop runs (Fig. \ref{fig_labview_scheme}).

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.5\linewidth]{LabVIEW_figs/HMI_RT_FPGA_Overview_1994019705.jpg}}
    \caption{Embedded system components \citep{ni_hmi_rt_fpga_comm}}
    \label{fig_labview_scheme}
\end{figure}

The implementation of the PID-F controller required modifications at each of these layers. At the FPGA level (Fig.\ref{fig_fpga_pmain}), the block responsible for computing the control action was modified to implement the desired PID-F structure.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.97\linewidth]{LabVIEW_figs/fpga_main.jpeg}}
    \caption{Total FPGA main block: FPGA\_main.vi }
    \label{fig_fpga_pmain}
\end{figure}

This proved non-trivial due to resource constraints on DSP48s operations within the FPGA. To overcome this, high-throughput math functions were employed for fixed-point arithmetic operations, with look-up tables used wherever possible to reduce DSP48s usage. The usage of LabVIEW FPGA high-throughout math blocks also implied the usage of fixed-point variables in the computations. As such, the controller gains, originally defined as 16-bit signed integers, were defined as 8-bit unsigned fixed-point values, with the bit allocation between integer and fractional parts chosen to maximise precision, while still offering the necessary operating range. This configuration allowed the modified control Virtual Instrument (VI) block (Fig. \ref{fig_fpga_pidf_v2}) to be successfully compiled and deployed to the FPGA.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.97\linewidth]{LabVIEW_figs/PIDF_V2.jpg}}
    \caption{Controller block: FPGA\_PIDF\_V2.vi }
    \label{fig_fpga_pidf_v2}
\end{figure}

Subsequently, the shared variables used for inter-layer communication (one of them shown in Fig. \ref{fig_fpga_control_params}) were updated to expose the PID-F controller gains, and the user interface was modified accordingly to allow the operator to set these gains during testing (Fig. \ref{fig_select_params}). This concludes the experimental implementation of the PID-F controller within the existing shaking table control software.

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.97\linewidth]{LabVIEW_figs/fpag_main_zoom.jpeg}}
    \caption{FPGA main block: "Control\_Params" cluster (containing the controller gains set by the user on the HMI) }
    \label{fig_fpga_control_params}
\end{figure}

\begin{figure}[H]
    \centering
    \fbox{\includegraphics[width=0.97\linewidth]{LabVIEW_figs/select_params.jpeg}}
    \caption{Controller gains menu on the HMI}
    \label{fig_select_params}
\end{figure}

Given the constraints on the availability of the hardware for implementation and testing, and the extensive modifications necessary at every layer of the software for implementation of the LQG servo controller, its implementation is pending.
...
The dynamic model identification of the plant is needed for tuning the controllers. To this end, multiple identification experiment were carried out using a limited-band pink noise driver signal, with amplitude set of approximately ±4 mm (according to the available displacement range of the shaker), using various controller settings, and data acquisition at 200 Hz. A sampling period of 5 ms is used throughout. Additionally, two identification experiments used a sine sweep signal of constant acceleration (amplitude) $\ddot{x} = 1.2 m/s^2$ as reference to drive the system.

The imperative of running experiments under closed-loop control poses additional challenges for identifying the plant (shaking table with physical model). To address the particularities of a closed-loop identification, the two-stage method \citep{vandenhof1993} illustrated in figure \ref{fig_two_stage_block_tikz} is applied. Firstly, the sensitivity function with controller, $S_0 C$, is identified by fitting a linear FIR model with 100 parameters (the order of the model just needs to be high enough to reproduce the noise-free control action, and high variance is not of concern). Then a Hammerstein-Wiener model is defined, using the previously identified FIR model as the linear block, and with the output nonlinearity equal to that of the physical system (saturation of the control action at known values), resulting in a more refined model for the sensitivity function. This function is then used to compute the noise-free control action $u_r$, from which (by being uncorrelated with the output y) the plant $G_0$ can now be estimated without bias introduced by the controller in the feedback loop.  The code implementation can be found at the following GitHub repository \href{https://github.com/c-radicallis/minimesa_data/blob/main/}{location}.  Identification was also performed by application of the direct method, and, since the controller was known, by indirect method. 
...
\begin{equation}
 J = \sum_{t=0}^{\infty}{{x_{aug}(t)}^\intercal Q} x_{aug}(t)+{u(t)}^\intercal R u(t) 
\end{equation}
...
\subsection{System identification, Controller Tuning, \& Performance Evaluation}
...
The indirect method yielded inconsistent results across experiments, and both the direct and two-stage methods produced mixed results. Based on the fundamental knowledge of applicability and limitations of each identification method in closed-loop systems, it was decided to proceed with the model identified via the two-stage method on the data from the run with the controller proportional gain set to $K_P = 10 V/m$.

The Bode plot of figure \ref{Fig6_Shaking table control loop frequency response with controllers optimized for the Tolmezzo earthquake} displays the result of the direct identification of the closed-loop with the controller set as proportional gain $K_p = 10 V/m$, and the expected closed-loop response with the respectively tuned controllers, computed from the plant model.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/newhall_20260525_1603/Response_Spectra_N.png}
...
\begin{subfigure}{0.49\textwidth}
    ...
    \caption{Displacement response spectrum}
    \label{Displacement response spectrum MSE for various seismic signals}
\end{subfigure}
\caption{Response spectrum MSE for various seismic signals}
\end{figure}
...
\chapter{Control Strategy Performance Evaluation for Benchmark Earthquakes with ST1D Model}\label{appendix:1DST_benchmark}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/elcentro_20250716_1639/Response_Spectra_N.png}
    \caption{Response spectra for El Centro fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/elcentro_20250716_1639/Response_Spectra_P.png}
    \caption{Response spectra for El Centro fault parallel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/erzikan_20250715_1123/Erzikan_Response_Spectra_N.png}
    \caption{Response spectra for Erzikan North-South}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/erzikan_20250715_1123/Erzikan_Response_Spectra_P.png}
    \caption{Response spectra for Erzikan East-West}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/kobe_20250715_1229/Response_Spectra_N.png}
    \caption{Response spectra for Kobe North-South}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/kobe_20250715_1229/Response_Spectra_P.png}
    \caption{Response spectra for Kobe East-West}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/laquila_20250709_1652/Response_Spectra_N.png}
    \caption{Response spectra for L'Aquila}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/tolmezzo_Response_Spectra_N.png}
    \caption{Response spectra for Tolmezzo}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/rinaldi_20250715_1517/Response_Spectra_N.png}
    \caption{Response spectra for Rinaldi fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/rinaldi_20250715_1517/Response_Spectra_P.png}
    \caption{Response spectra for Rinaldi fault parallel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/newhall_20250715_1236/Response_Spectra_N.png}
    \caption{Response spectra for Newhall fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/newhall_20250715_1236/Response_Spectra_P.png}
    \caption{Response spectra for Newhall fault parallel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/sylmar_20250722_1937/Response_Spectra_N.png}
    \caption{Response spectra for Sylmar fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{1DST_benchmark/sylmar_20250722_1937/Response_Spectra_P.png}
    \caption{Response spectra for Sylmar fault parallel}
\end{figure}

\chapter{Electrodynamic Shaker Identification Results}\label{appendix:full_shaker_id_results}

\begin{figure}[H]
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/direct_full.jpg}
    \caption{Direct method}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/indirect_full.jpg}
    \caption{Indirect method}
\end{subfigure}
\\
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/twostage_full.jpg}
    \caption{Two-stage method}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/cl_full.jpg}
    \caption{Direct identification of closed-loop}
\end{subfigure}
\caption{Bode plot of identification results with different methods.  The label "sweep" denotes the experiments driven with a sine sweep. "P" denotes the proportional gain for which the controller was set, and "wc10", "wc15", "wc20" denotes the PIDF tunes described in figures \ref{wc10},\ref{wc15}, and \ref{wc20}, respectively.}
\end{figure}

\begin{figure}[H]
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/1.jpg}
    \caption{Sine sweep with controller $K_P = 7$}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/2.jpg}
    \caption{Sine sweep with controller $K_P = 15$}
\end{subfigure}
\\
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/3.jpg}
    \caption{Pink noise with controller $K_P = 5$}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/4.jpg}
    \caption{Pink noise with controller $K_P = 7$}
\end{subfigure}
\\
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/5.jpg}
    \caption{Pink noise with controller $K_P = 10$}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/6.jpg}
    \caption{Pink noise with controller $K_P = 15$}
\end{subfigure}
\caption{ Autocorrelation of the residuals, and cross-correlation between the residuals and the inputs, for each identified model, for each experiment}
\end{figure}
...
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/7.jpg}
    \caption{Pink noise with controller $K_p = 6.75 ,\,  K_i = 68 ,\,  K_d = .0712891 ,\,  T_f = 0.00161743$}\label{wc10}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/8.jpg}
    \caption{Pink noise with controller $K_p = 8.75,\,  K_i = 139 ,\,  K_d = 0.125977 ,\,  T_f = 0.00109863$}\label{wc15}
\end{subfigure}
\\
\begin{subfigure}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{shaker_id_results/20260529_1749/9.jpg}
    \caption{Pink noise with controller $K_p = 8.5625 ,\,  K_i = 75 ,\,  K_d = 0.18457 ,\,  T_f = 0.000167847$}\label{wc20}
\end{subfigure}

\caption{ Autocorrelation of the residuals, and cross-correlation between the residuals and the inputs, for each identified model, for each experiment}
\end{figure}

\chapter{Control Strategy Performance Evaluation for Benchmark Earthquakes with Electrodynamic Shaker Model}\label{appendix:shaker_benchmark}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/elcentro_20260424_1833/Response_Spectra_N.png}
    \caption{Response spectra for El Centro fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/elcentro_20260424_1833/Response_Spectra_P.png}
    \caption{Response spectra for El Centro fault parallel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/erzikan_20260427_1244/Response_Spectra_N.png}
    \caption{Response spectra for Erzikan North-South}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/erzikan_20260427_1244/Response_Spectra_P.png}
    \caption{Response spectra for Erzikan East-West}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/kobe_20260427_1452/Response_Spectra_N.png}
    \caption{Response spectra for Kobe North-South}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/kobe_20260427_1452/Response_Spectra_P.png}
    \caption{Response spectra for Kobe East-West}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/laquila_Response_Spectra_N.png}
    \caption{Response spectra for L'Aquila}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/tolmezzo_Response_Spectra_N.png}
    \caption{Response spectra for Tolmezzo}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/rinaldi_20260428_1809/Response_Spectra_N.png}
    \caption{Response spectra for Rinaldi fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/rinaldi_20260428_1809/Response_Spectra_P.png}
    \caption{Response spectra for Rinaldi fault parallel}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/newhall_20260525_1603/Newhall_FP.jpg}
    \caption{Response spectra for Newhall fault parallel}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/sylmar_20260427_1718/Response_Spectra_N.png}
    \caption{Response spectra for Sylmar fault normal}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{Shaker_benchmark/sylmar_20260427_1718/Response_Spectra_P.png}
    \caption{Response spectra for Sylmar fault parallel}
\end{figure}
